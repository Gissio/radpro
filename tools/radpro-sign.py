...
def sign_firmware(env_name):
    ...
    for language in languages:
        ...
        build_dir = get_radpro_path() + "platform.io/.pio/build/" + build_name + "/"
        ...
        firmware_offset, firmware_size, image_size, states_base, states_size = (
            extract_elf_data(elf_file)
        )
        ...
        write_word(
            image, firmware_footer_offset, get_crc(image[0:firmware_footer_offset])
        )

        # Truncate if not FNIRSI GC-01
        if not env_name.startswith("fnirsi-gc01"):
            image = image[0:firmware_size]
